...
socket.on('createGroup', ({ groupName, invited, username }) => {
    // Step 1: Find user IDs for invited usernames and the creator
    const placeholders = invited.map(() => '?').join(',');
    const findUsersSQL = `SELECT id, username, socketId FROM users WHERE username IN (${placeholders}, ?)`;
    
    db.all(findUsersSQL, [...invited, username], (err, rows) => {
        if (err) {
            console.error("Error fetching user IDs:", err);
            return;
        }

        // Step 2: Map usernames to their corresponding IDs
        const userIds = {};
        const socketIds = {}; // Collect socket IDs to send WebSocket messages

        rows.forEach(row => {
            userIds[row.username] = row.id;
            socketIds[row.username] = row.socketId; // Store socketId for WebSocket notifications
        });

        // Ensure the creator's ID is found
        if (!userIds[username]) {
        ...
        db.run(insertGroupSQL, [userIds[username], groupName], function(err) {
            ...
            // Step 4: Insert invites for each invited user into `groupInvite`
            ...
            invited.forEach(invitedUser => {
                db.run(insertInviteSQL, [userIds[username], userIds[invitedUser], groupId, groupName, 0], err => {
                    if (err) {
                        ...
                        // Notify invited users via WebSocket
                        const invitedSocketId = socketIds[invitedUser]; // Get socketId from the map
                        if (invitedSocketId) {
            ...
            // Step 5: Insert the row for the creator and mark it as accepted
