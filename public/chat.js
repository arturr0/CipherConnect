...
// Add resize event listener
    ...
    socket.on('groupInvites', (data) => {
        ...
        data.forEach(invite => {
            ...
            invitation.classList.add('groupInvitation');
            ...
            let invitaionValue = parseInt(groupCounter.getAttribute('value'), 10) || 0; // Default to 0 if NaN
            invitaionValue++;
            console.log(invitaionValue);
            groupCounter.setAttribute('value', invitaionValue);
            groupCounter.textContent = invitaionValue;
            document.getElementById("groupsContent").appendChild(invitation);
        })
    });
